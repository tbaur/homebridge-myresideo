...
> The account-linking UI is on the roadmap (see [`ROADMAP.md`](ROADMAP.md)). Until then, tokens can be obtained manually following [`API.md`](API.md).
...
- **Rotation persistence** — when Resideo returns a new `refresh_token`, it is written back to `config.json` so it survives a restart.
- **Clear failure** — a `400 invalid_grant` surfaces as `RefreshTokenInvalidError` with a log message telling the user to re-link, instead of silently looping.
