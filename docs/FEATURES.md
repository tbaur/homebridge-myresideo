...
- ✅ Health diagnostics (`diagnosticsInterval`, default 10800 / 3 hours; `0` disables): a periodic heartbeat in the sibling-plugin shape (`devices | rest <state> | api p50/p95 (req, err)`), where REST state is a real lifecycle (`live` while polling, `connecting` before the poll loop is armed, `auth-failed` during token-refresh cooldown, `stopped` on shutdown). `healthy`/`degraded` rollup covers circuit breaker open, high API error rate, token-refresh failure, empty-discovery retry, or a fully-failed poll cycle. An active leak and a non-CLOSED breaker appear on the human line only when relevant; boot/shutdown snapshots, healthy↔degraded transition logs, and optional structured-JSON output (`structuredLogs`) carry the full counters
...
- ✅ Refresh + access tokens persisted atomically to a plugin-owned file under Homebridge storage after every successful refresh (cross-platform replace; `config.json` is not rewritten)
...
│   ├── diagnostics/      # Health/activity metrics collector
