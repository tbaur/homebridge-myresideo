...
- ✅ Health diagnostics (`diagnosticsInterval`, default 10800 / 3 hours; `0` disables): a periodic heartbeat reporting device online count, poll success/failure, and API latency (p50/p95), with a `healthy`/`degraded` rollup (circuit breaker open, high API error rate, token-refresh failure, empty-discovery retry, or a fully-failed poll cycle). An active leak and a non-CLOSED breaker appear on the human line only when relevant; boot/shutdown snapshots, healthy↔degraded transition logs, and optional structured-JSON output (`structuredLogs`) carry the full counters
