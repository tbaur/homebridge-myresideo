...
| **Structured Diagnostics** | ✅ | Diagnostics subsystem (`diagnosticsInterval`, schema default 10800 / 3 hours; `0` disables): a quiet human heartbeat (devices, poll outcome, API p50/p95) that surfaces leak count and breaker state only when they carry signal; full counters (token expiry, breaker trips, etc.) stay in the optional structured JSON; `healthy`/`degraded` rollup (including `circuitBreakerOpen`); boot/shutdown snapshots carry a redacted config echo. All emission is failure-isolated so it can never crash the host |
