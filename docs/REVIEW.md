...
The plugin targets a single device type (the WiFi Water Leak & Freeze Detector) over a poll-only REST API, and is intentionally kept small, with a dependency-free runtime core (the optional account-linking UI adds a single, dependency-free package). A diagnostics subsystem (periodic health heartbeat with optional structured JSON logging; schema and runtime default `0` / off) keeps the steady-state footprint low. Adding support for other Honeywell Home device types is outlined in [`DEVELOPMENT.md`](../DEVELOPMENT.md).
