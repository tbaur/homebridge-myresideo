...
| **Config Schema ↔ Validators** | ✅ | `validateConfig` enforces every field (including credentials) at startup; `config.schema.json` declares both the user-editable options and the `credentials` object (so config-ui-x preserves the saved tokens), but keeps `credentials` out of the rendered `layout` so they are managed by the account-linking UI rather than shown in the form. A schema regression test (`tests/unit/config-schema.test.ts`) locks in this contract, and additionally asserts the schema stays valid draft-07 JSON Schema — `required` declared only as arrays (never a boolean on a field), with `name` and per-device `deviceID` required — so config-ui-x can always compile and validate it |
