...
import { resolveTokenStorePath } from '../../src/api/token-store'
...
const TEST_STORAGE = '/tmp/hb-storage'
...
function makeApi() {
  ...
  const api = {
    ...
    user: {
      configPath: jest.fn(() => '/tmp/config.json'),
      storagePath: jest.fn(() => TEST_STORAGE),
    },
  }
  return { api, handlers }
}
...
function defaultStorePath(name = 'MyResideo', consumerKey = 'key'): string {
  return resolveTokenStorePath(TEST_STORAGE, name, consumerKey)
}

/** Build a platform on the mocked filesystem and run one token rotation through it. */
async function rotateToken(log: Logging, config = validConfig()): Promise<API> {
  const { api } = makeApi()
  new ResideoPlatform(log, config, api as unknown as API)

  const calls = (TokenManager as unknown as jest.Mock).mock.calls
  const tokenOpts = calls[calls.length - 1][0] as {
...
describe('refresh token persistence', () => {
  it('writes the rotated token to the plugin store, not config.json', async () => {
    const log = makeLog()
    const api = await rotateToken(log)
    const destPath = defaultStorePath()
    ...
    expect(content).toContain('"sourceRefreshToken": "refresh"')
    ...
    expect(api.user.configPath).not.toHaveBeenCalled()
  })

  it('starts TokenManager from the store when the record belongs to this link', () => {
    ...
    new ResideoPlatform(makeLog(), validConfig(), makeApi().api as unknown as API)

    const tokenOpts = (TokenManager as unknown as jest.Mock).mock.calls[0][0] as {
      refreshToken: string
      accessToken: string
    }
    expect(tokenOpts.refreshToken).toBe('rotated')
    expect(tokenOpts.accessToken).toBe('access-stored')
  })

  it('starts TokenManager from config when the store is from a previous link', () => {
    mockReadFileSync.mockReturnValue(JSON.stringify({
      version: 1,
      refreshToken: 'stale-rotated',
      accessToken: 'stale-access',
      sourceRefreshToken: 'older-link',
      updatedAt: 1,
    }))
    ...
    expect(tokenOpts.refreshToken).toBe('refresh')
    expect(tokenOpts.accessToken).toBeUndefined()
  })

  it('writes each instance to its own store file', async () => {
    const kitchen = validConfig()
    kitchen.name = 'Kitchen'
    const garage = validConfig()
    garage.name = 'Garage'

    await rotateToken(makeLog(), kitchen)
    const kitchenTemp = mockOpen.mock.calls[0][0] as string
    expect(mockRename).toHaveBeenCalledWith(kitchenTemp, defaultStorePath('Kitchen'))

    mockOpen.mockClear()
    mockRename.mockClear()
    await rotateToken(makeLog(), garage)
    const garageTemp = mockOpen.mock.calls[0][0] as string
    expect(mockRename).toHaveBeenCalledWith(garageTemp, defaultStorePath('Garage'))
    expect(defaultStorePath('Kitchen')).not.toBe(defaultStorePath('Garage'))
  })

  it('does not throw when persistence fails', async () => {
    mockHandleSync.mockRejectedValue(new Error('disk gone'))

    const log = makeLog()
    await expect(rotateToken(log)).resolves.toBeTruthy()
    expect(log.error).toHaveBeenCalledWith(expect.stringContaining('Could not persist'))
  })

  it('skips persist when Homebridge gave no storage directory', async () => {
    const log = makeLog()
    const { api } = makeApi()
    api.user.storagePath = jest.fn(() => '')
    new ResideoPlatform(log, validConfig(), api as unknown as API)
    ...
    expect(api.user.configPath).not.toHaveBeenCalled()
    expect(log.error).toHaveBeenCalledWith(expect.stringContaining('storage directory'))
  })
})
