...
- **Self-healing discovery** — if initial discovery fails on a transient error, it retries with jittered capped exponential backoff (15s → 5min). Non-recoverable errors (invalid refresh token, rejected credentials) are not retried. An HTTP 200 that returns zero detectors while cached accessories remain is treated the same way (warn, skip stale removal, restore handlers from cache when `locationId` is known, retry) so a Resideo outage cannot wipe HomeKit. In-flight poll workers stop claiming new devices once shutdown begins.
